...
const getRapportRje = async (req, res) => {
  try {
    ...
    return res.status(HttpStatus.OK).json(finalData);
  } catch (error) {
    console.error(error);
    return res
      ...
      engins.forEach((engin) => {
        if (engin.Saisiehrm) {
          const hasSaisieInPeriod = engin.Saisiehrm.some(
            (s) => s.du >= periodStart && s.du <= periodEnd
          );
...
const getIndispoParParc = async (req, res) => {
  try {
    ...
    const calculateIndicators = (engins, periodStart, periodEnd) => {
      ...
      engins.forEach((engin) => {
        if (engin.Saisiehrm && Array.isArray(engin.Saisiehrm)) {
          const hasSaisieInPeriod = engin.Saisiehrm.some(
            (saisie) => saisie.du >= periodStart && saisie.du <= periodEnd
          );
    ...
    parcs.forEach((parc) => {
      ...
      const daysInMonth =
        Math.floor((lastDayOfMonth - firstDayOfMonth) / (1000 * 60 * 60 * 24)) +
        1;
      const daysInYear =
        Math.floor((lastDayOfMonth - firstDayOfYear) / (1000 * 60 * 60 * 24)) +
        1;
  ...
  try {
    ...
    if (!typelubrifiant) {
      return res
        ...
        .json({ message: "Typelubrifiant not found" });
    }
  ...
  try {
    ...
    if (!parcId || !date) {
      return res
        ...
        .json({ error: "parcId et date sont obligatoires" });
    }
...
const getAnalyseSpcPeriodParcTypeConsomm = async (req, res) => {
  try {
    ...
    // 1. Récupérer les lubrifiants du type et du parc
    ...
    const result = lubrifiants.map((lub) => {
      const total = lub.Saisielubrifiant.reduce((sum, s) => sum + s.qte, 0);
      return {
        name: lub.name,
        sum: total,
      };
    ...
    const totalGeneral = result.reduce((sum, item) => sum + item.sum, 0);

    // 4. Ajouter les pourcentages
    const finalResult = result
      .map((item) => ({
        ...
        percentage:
          totalGeneral > 0
            ? parseFloat(((item.sum / totalGeneral) * 100).toFixed(2))
            : 0,
      }))
...
const getIndispoParcPeriode = async (req, res) => {
  try {
    ...
    if (!parcId || !dateDu || !dateAu) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: "parcId, dateDu et dateAu sont requis" });
    }
