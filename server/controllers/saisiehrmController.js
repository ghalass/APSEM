const prisma = require("../config/prismaClient");
const HttpStatus = require("../helpers/httpStatus");
const get_byengin_and_date = async (req, res) => {
  try {
    const { du, enginId } = req.body;

    const saisieHRM = await prisma.saisiehrm.findMany({
      where: {
        enginId: enginId,
        du: new Date(du),
      },
    });

    const saisieHIM = await prisma.saisiehim.findMany({
      where: {
        enginId: enginId,
        du: new Date(du),
      },
    });

    res.status(HttpStatus.OK).json({
      saisieHRM,
      saisieHIM,
    });
  } catch (error) {
    res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ error: error.message });
  }
};

const createSaisieHrm = async (req, res) => {
  try {
    const { du, enginId, siteId, hrm } = req.body;
    // Vérification des champs obligatoires
    const missingFields = ["du", "enginId", "siteId", "hrm"].filter(
      (field) => !req.body[field]
    );
    if (missingFields.length > 0) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: "Veuillez remplir tous les champs!", missingFields });
    }
    // check if already exist
    exist = await prisma.saisiehrm.findFirst({
      where: { du: new Date(du), enginId: enginId },
    });

    if (isNaN(hrm) || hrm > 24 || hrm < 0)
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: `HRM ne doit pas depasser 24h`, hrm });

    if (exist)
      return res.status(HttpStatus.BAD_REQUEST).json({
        error: `Saisie déjà faite pour cet engin à cette date!`,
        exist,
      });
    const savedSaisie = await prisma.saisiehrm.create({
      data: {
        du: new Date(du),
        enginId: enginId,
        siteId: siteId,
        hrm: parseFloat(hrm),
      },
    });
    return res.status(HttpStatus.CREATED).json(savedSaisie);
  } catch (error) {
    console.log(error);
  }
};

const updateSaisieHrm = async (req, res) => {
  try {
    const { id, hrm } = req.body;
    // Vérification des champs obligatoires
    const missingFields = ["id", "hrm"].filter((field) => !req.body[field]);
    if (missingFields.length > 0) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: "Veuillez remplir tous les champs!", missingFields });
    }

    // check if already exist
    exist = await prisma.saisiehrm.findFirst({
      where: { id },
    });
    if (!exist)
      return res
        .status(HttpStatus.NOT_FOUND)
        .json({ error: "Saisie n'existe pas!", exist });

    // CHECK TOTAL HRM & HIM
    const totlaHRM = await prisma.saisiehrm.aggregate({
      _sum: { hrm: true },
      where: { id },
    });
    const totlaHIM = await prisma.saisiehim.aggregate({
      _sum: { him: true },
      where: { saisiehrmId: id },
    });
    const him_hrm_saisie = totlaHIM._sum.him + Number(hrm);
    let message = `HRM saisie = ${totlaHRM._sum.hrm || 0}\n`;
    message += `HIM saisie = ${totlaHIM._sum.him || 0}\n`;
    message += `Nouveau HRM = ${hrm}\n`;
    message += `Total sera = ${him_hrm_saisie} > 24h\n`;
    message += `** IMPOSSIBLE de dépasser 24h **`;
    if (him_hrm_saisie > 24)
      return res.status(HttpStatus.BAD_REQUEST).json({ error: message });

    const HrmToUpdate = await prisma.saisiehrm.update({
      where: { id },
      data: { hrm: parseFloat(hrm) },
    });
    return res.status(HttpStatus.CREATED).json(HrmToUpdate);
  } catch (error) {
    console.log(error);
  }
};

const createSaisieHim = async (req, res) => {
  try {
    const { panneId, him, ni, saisiehrmId } = req.body;
    // Vérification des champs obligatoires
    const missingFields = ["panneId", "him", "ni", "saisiehrmId"].filter(
      (field) => !req.body[field]
    );
    if (missingFields.length > 0) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: "Veuillez remplir tous les champs!", missingFields });
    }
    // check if panneId exist
    panneExist = await prisma.panne.findFirst({
      where: { id: panneId },
    });
    if (!panneExist)
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: "Panne n'existe pas", panneId });

    // check if already exist
    exist = await prisma.saisiehim.findFirst({
      where: { panneId: panneId, saisiehrmId: saisiehrmId },
    });
    if (exist)
      return res.status(HttpStatus.BAD_REQUEST).json({
        error: "Saisie déjà faite pour cet engin à cette date!",
        exist,
      });

    // CHECK TOTAL HRM & HIM
    const totlaHRM = await prisma.saisiehrm.aggregate({
      _sum: { hrm: true },
      where: { id: saisiehrmId },
    });
    const totlaHIM = await prisma.saisiehim.aggregate({
      _sum: { him: true },
      where: { saisiehrmId: saisiehrmId },
    });
    const him_hrm_saisie = totlaHRM._sum.hrm + totlaHIM._sum.him + Number(him);
    let message = `HRM saisie = ${totlaHRM._sum.hrm || 0}\n`;
    message += `HIM saisie = ${totlaHIM._sum.him || 0}\n`;
    message += `Nouveau HIM = ${him || 0}\n`;
    message += `Total sera = ${him_hrm_saisie} > 24h\n`;
    message += `** IMPOSSIBLE de dépasser 24h **`;
    if (him_hrm_saisie > 24)
      return res.status(HttpStatus.BAD_REQUEST).json({ error: message });

    const savedSaisie = await prisma.saisiehim.create({
      data: {
        panneId: panneId,
        him: parseFloat(him),
        ni: parseInt(ni),
        saisiehrmId: saisiehrmId,
      },
    });
    return res.status(HttpStatus.CREATED).json(savedSaisie);
  } catch (error) {
    console.log(error);
  }
};

const deleteSaisieHim = async (req, res) => {
  try {
    const { id } = req.body;
    // Vérification des champs obligatoires
    const missingFields = ["id"].filter((field) => !req.body[field]);
    if (missingFields.length > 0) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: "Veuillez remplir tous les champs!", missingFields });
    }
    // check if saisiehim exist
    saisiehimExist = await prisma.saisiehim.findFirst({
      where: { id },
    });
    if (!saisiehimExist)
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: "Panne n'existe pas", panneId });

    const saisiehimToDelete = await prisma.saisiehim.delete({
      where: { id },
    });
    return res.status(HttpStatus.CREATED).json(saisiehimToDelete);
  } catch (error) {
    console.log(error);
  }
};

const updateSaisieHim = async (req, res) => {
  try {
    console.log(req.body);

    const { id, panneId, him, ni, saisiehrmId } = req.body;
    // Vérification des champs obligatoires
    const missingFields = ["id", "panneId", "him", "ni", "saisiehrmId"].filter(
      (field) => !req.body[field]
    );
    console.log(missingFields);

    if (missingFields.length > 0) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: "Veuillez remplir tous les champs!", missingFields });
    }

    // check if already exist
    existSaisiehim = await prisma.saisiehim.findFirst({
      where: { id },
    });
    if (!existSaisiehim)
      return res
        .status(HttpStatus.NOT_FOUND)
        .json({ error: "Saisie n'existe pas!", exist });

    // CHECK TOTAL HRM & HIM
    const totlaHRM = await prisma.saisiehrm.aggregate({
      _sum: { hrm: true },
      where: { id: saisiehrmId },
    });
    const totlaHIM = await prisma.saisiehim.aggregate({
      _sum: { him: true },
      where: { saisiehrmId: saisiehrmId },
    });
    const him_hrm_saisie = totlaHRM._sum.hrm + totlaHIM._sum.him + Number(him);
    let message = `HRM saisie = ${totlaHRM._sum.hrm || 0}\n`;
    message += `HIM saisie = ${totlaHIM._sum.him || 0}\n`;
    message += `Nouveau HIM = ${him || 0}\n`;
    message += `Total sera = ${him_hrm_saisie} > 24h\n`;
    message += `** IMPOSSIBLE de dépasser 24h **`;
    if (him_hrm_saisie > 24)
      return res.status(HttpStatus.BAD_REQUEST).json({ error: message });

    const updated = await prisma.saisiehim.update({
      where: { id },
      data: {
        panneId: panneId,
        him: parseFloat(him),
        ni: parseInt(ni),
      },
    });
    return res.status(HttpStatus.CREATED).json(updated);
  } catch (error) {
    console.log(error);
  }
};

const getSaisieHrm = async (req, res) => {
  try {
    const { du, enginId } = req.body;
    // Vérification des champs obligatoires
    const missingFields = ["du", "enginId"].filter((field) => !req.body[field]);
    if (missingFields.length > 0) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: "Veuillez remplir tous les champs!", missingFields });
    }

    const startDate = new Date(du); // 2025-03-29T00:00:00.000Z
    const endDate = new Date(startDate);
    endDate.setDate(endDate.getDate() + 1); // 2025-03-30T00:00:00.000Z

    const saisiehrm = await prisma.saisiehrm.findMany({
      where: {
        du: {
          gte: startDate, // Greater than or equal to start of day
          lt: endDate, // Less than start of next day
        },
        enginId: enginId,
      },
      include: {
        Saisiehim: {
          include: {
            Panne: { include: { Typepanne: true } },
            Saisielubrifiant: {
              include: {
                Lubrifiant: { include: { Typelubrifiant: true } },
                Typeconsommationlub: true,
              },
            },
          },
        },
        Engin: true,
        Site: true,
      },
      orderBy: { du: "desc" },
    });
    return res.status(HttpStatus.OK).json(saisiehrm);
  } catch (error) {
    console.log(error);
  }
};

const getSaisieHrmDay = async (req, res) => {
  try {
    const { du } = req.body;

    // Convertir la date en objet Date
    const dateDu = new Date(du);

    // Calculer le premier et le dernier jour du mois
    const firstDayOfMonth = new Date(
      dateDu.getFullYear(),
      dateDu.getMonth(),
      1
    );
    const lastDayOfMonth = new Date(
      dateDu.getFullYear(),
      dateDu.getMonth() + 1,
      0
    );

    firstDayOfMonth.setHours(0, 0, 0, 0); // 00:00:00.000
    lastDayOfMonth.setHours(23, 59, 59, 999); // 23:59:59.999

    // Récupérer toutes les saisies HRM pour la période avec les relations nécessaires
    const saisies = await prisma.saisiehrm.findMany({
      where: {
        du: {
          gte: firstDayOfMonth,
          lte: lastDayOfMonth,
        },
      },
      include: {
        Engin: {
          include: {
            Parc: {
              include: {
                Typeparc: true,
              },
            },
          },
        },
        Site: true,
        Saisiehim: {
          include: {
            Panne: true,
            Saisielubrifiant: {
              include: {
                Lubrifiant: true,
                Typeconsommationlub: true,
              },
            },
          },
        },
      },
      orderBy: {
        du: "asc", // Tri par date croissante
      },
    });

    // Préparer le résultat final
    const result = [];

    // Traiter chaque saisie HRM
    for (const saisie of saisies) {
      const baseData = {
        date: saisie.du.toISOString().split("T")[0], // Format YYYY-MM-DD
        typeparc: saisie.Engin.Parc.Typeparc.name,
        parc: saisie.Engin.Parc.name,
        engin: saisie.Engin.name,
        site: saisie.Site.name,
        hrm: saisie.hrm,
      };

      // Si l'engin a des pannes (Saisiehim)
      if (saisie.Saisiehim && saisie.Saisiehim.length > 0) {
        for (const saisieHim of saisie.Saisiehim) {
          // Préparer les lubrifiants consommés
          const lubrifiants = saisieHim.Saisielubrifiant.map((lub) => ({
            name: lub.Lubrifiant.name,
            qte: lub.qte,
            typeConsommation: lub.Typeconsommationlub?.name || null,
          }));

          result.push({
            ...baseData,
            panne: saisieHim.Panne.name,
            him: saisieHim.him,
            ni: saisieHim.ni,
            obs: saisieHim.obs || null,
            lubrifiants: lubrifiants,
          });
        }
      } else {
        // Si l'engin n'a pas de pannes
        result.push({
          ...baseData,
          panne: null,
          him: 0,
          ni: 0,
          obs: null,
          lubrifiants: [],
        });
      }
    }

    return res.status(HttpStatus.OK).json(result);
  } catch (error) {
    console.error("Erreur:", error);
    return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
      message: "Erreur serveur",
      error: error.message,
    });
  }
};

//
const injectSaisieHrm = async (req, res) => {
  try {
    let saisieInputs = { du: null, enginId: null, siteId: null, hrm: null };

    if (req.body?.DATE_RJE === undefined)
      return res
        .status(HttpStatus.OK)
        .json({ message: `NOK : Colonne DATE_RJE introuvable.` });
    if (req.body?.ENGIN === undefined)
      return res
        .status(HttpStatus.OK)
        .json({ message: `NOK : Colonne ENGIN introuvable.` });
    if (req.body?.SITE === undefined)
      return res
        .status(HttpStatus.OK)
        .json({ message: `NOK : Colonne SITE introuvable.` });
    if (req.body?.HRM === undefined)
      return res
        .status(HttpStatus.OK)
        .json({ message: `NOK : Colonne HRM introuvable.` });

    const engin = await prisma.engin.findFirst({
      where: { name: req.body?.ENGIN },
    });
    const site = await prisma.site.findFirst({
      where: { name: req.body?.SITE },
    });

    saisieInputs.du = req.body?.DATE_RJE || null;
    saisieInputs.enginId = engin?.id || null;
    saisieInputs.siteId = site?.id || null;
    saisieInputs.hrm = req.body?.HRM || null;

    const missingFields = ["du", "enginId", "siteId", "hrm"].filter(
      (field) => !saisieInputs[field]
    );
    if (missingFields.length > 0) {
      return res.status(HttpStatus.OK).json({
        message: `${req.body?.ENGIN} NOK : Veuillez remplir tous les champs!.`,
      });
    }

    if (
      isNaN(saisieInputs.hrm) ||
      saisieInputs.hrm > 24 ||
      saisieInputs.hrm < 0
    )
      return res.json({
        message: `${req.body?.ENGIN} NOK : HRM ne doit pas depasser 24h`,
      });
    exist = await prisma.saisiehrm.findFirst({
      where: {
        du: new Date(saisieInputs.du),
        enginId: saisieInputs.enginId,
      },
    });
    if (exist)
      return res.status(HttpStatus.OK).json({
        message: `${req.body?.ENGIN} NOK : Saisie déjà faite pour cet engin à cette date!`,
      });

    await prisma.saisiehrm.create({
      data: {
        du: new Date(saisieInputs.du),
        enginId: saisieInputs.enginId,
        siteId: saisieInputs.siteId,
        hrm: parseFloat(saisieInputs.hrm),
      },
    });

    return res
      .status(HttpStatus.OK)
      .json({ message: `${req.body?.ENGIN} OK : Saisie faite` });
  } catch (error) {
    console.log(error);
  }
};

const injectSaisieHim = async (req, res) => {
  try {
    let saisieInputs = {
      panneId: null,
      him: null,
      ni: null,
      saisiehrmId: null,
    };

    if (req.body?.DATE_RJE === undefined)
      return res
        .status(HttpStatus.OK)
        .json({ message: `NOK : Colonne DATE_RJE introuvable.` });
    if (req.body?.ENGIN === undefined)
      return res
        .status(HttpStatus.OK)
        .json({ message: `NOK : Colonne ENGIN introuvable.` });
    if (req.body?.PANNE === undefined)
      return res
        .status(HttpStatus.OK)
        .json({ message: `NOK : Colonne PANNE introuvable.` });
    if (req.body?.HIM === undefined)
      return res
        .status(HttpStatus.OK)
        .json({ message: `NOK : Colonne HIM introuvable.` });
    if (req.body?.NI === undefined)
      return res
        .status(HttpStatus.OK)
        .json({ message: `NOK : Colonne NI introuvable.` });

    const engin = await prisma.engin.findUnique({
      where: { name: req.body?.ENGIN },
    });
    if (!engin)
      return res
        .status(HttpStatus.OK)
        .json({ message: `${req.body?.ENGIN} NOK : Engin introuvable!.` });

    const panne = await prisma.panne.findFirst({
      where: {
        name: req.body?.PANNE,
        Typepanne: {
          TypepanneParc: {
            some: {
              parcId: engin.parcId,
            },
          },
        },
      },
      select: { id: true },
    });
    if (!panne)
      return res.status(HttpStatus.OK).json({
        message: `${engin?.name}->${req.body?.PANNE}-> NOK : Code Panne introuvable!.`,
      });

    const saisieHrm = await prisma.saisiehrm.findUnique({
      where: { du: new Date(req.body?.DATE_RJE), enginId: engin?.id },
    });
    if (!saisieHrm)
      return res
        .status(HttpStatus.OK)
        .json({ message: `${req.body?.ENGIN}-> NOK : Aucune HRM saisie!.` });

    saisieInputs.panneId = panne?.id || null;
    saisieInputs.him = req.body?.HIM || null;
    saisieInputs.ni = req.body?.NI || null;
    saisieInputs.saisiehrmId = saisieHrm?.hrm || null;

    const missingFields = ["panneId", "him", "ni", "saisiehrmId"].filter(
      (field) => !saisieInputs[field]
    );
    if (missingFields.length > 0) {
      return res.status(HttpStatus.OK).json({
        message: `${req.body?.ENGIN}-> NOK : Veuillez remplir tous les champs!.`,
      });
    }

    return res.status(HttpStatus.OK).json({
      message: `${engin?.name}->${req.body?.PANNE}-> OK : Saisie faite`,
    });
    /*** */
    const { panneId, him, ni, saisiehrmId } = req.body;
    // // Vérification des champs obligatoires
    // const missingFields = ["panneId", "him", "ni", "saisiehrmId"].filter((field) => !req.body[field]);
    // if (missingFields.length > 0) {
    //     return res
    //         .status(HttpStatus.BAD_REQUEST)
    //         .json({ error: "Veuillez remplir tous les champs!", missingFields });
    // }
    // check if panneId exist
    panneExist = await prisma.panne.findFirst({
      where: { id: panneId },
    });
    if (!panneExist)
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: "Panne n'existe pas", panneId });

    // check if already exist
    exist = await prisma.saisiehim.findFirst({
      where: { panneId: panneId, saisiehrmId: saisiehrmId },
    });
    if (exist)
      return res.status(HttpStatus.BAD_REQUEST).json({
        error: "Saisie déjà faite pour cet engin à cette date!",
        exist,
      });

    // CHECK TOTAL HRM & HIM
    const totlaHRM = await prisma.saisiehrm.aggregate({
      _sum: { hrm: true },
      where: { id: saisiehrmId },
    });
    const totlaHIM = await prisma.saisiehim.aggregate({
      _sum: { him: true },
      where: { saisiehrmId: saisiehrmId },
    });
    const him_hrm_saisie = totlaHRM._sum.hrm + totlaHIM._sum.him + Number(him);
    let message = `HRM saisie = ${totlaHRM._sum.hrm || 0}\n`;
    message += `HIM saisie = ${totlaHIM._sum.him || 0}\n`;
    message += `Nouveau HIM = ${him || 0}\n`;
    message += `Total sera = ${him_hrm_saisie} > 24h\n`;
    message += `** IMPOSSIBLE de dépasser 24h **`;
    if (him_hrm_saisie > 24)
      return res.status(HttpStatus.BAD_REQUEST).json({ error: message });

    const savedSaisie = await prisma.saisiehim.create({
      data: {
        panneId: panneId,
        him: parseFloat(him),
        ni: parseInt(ni),
        saisiehrmId: saisiehrmId,
      },
    });
    return res.status(HttpStatus.CREATED).json(savedSaisie);
  } catch (error) {
    console.log(error);
  }
};

module.exports = {
  get_byengin_and_date,

  createSaisieHrm,
  updateSaisieHrm,

  createSaisieHim,
  deleteSaisieHim,
  updateSaisieHim,

  getSaisieHrm,

  getSaisieHrmDay,

  injectSaisieHrm,
  injectSaisieHim,
};
